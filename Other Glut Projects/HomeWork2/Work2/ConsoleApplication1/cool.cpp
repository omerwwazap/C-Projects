...
float castelX, castelX2, castelX3, castelX4, castelX5 = -180;     //location of the castel
float castelSpd, castelSpd2, castelSpd3, castelSpd4, castelSpd5 = 3; //castel speed
	...
	if (gamestate) {
		...
		if (!gameover) {
			if (animation) {
				...
				if ((Cy == (hity1 + hity2) / 2) || (Cy == (hity12 + hity22) / 2)) {
					if ((Cx >= hitx1 && Cx <= hitx2) || (Cx >= hitx12 && Cx <= hitx22)) {
						point[0] = point[0] + 5;
						point[1] = 5;
						resetcastel();
					}
				...
				if ((Cy >= hity1 && Cy <= hity2) || (Cy >= hity12 && Cy <= hity22)) {
					if ((Cx >= hitx1 && Cx <= hitx2) || (Cx >= hitx12 && Cx <= hitx22)) {
						point[0] = point[0] + 3;
						resetcastel();
						point[1] = 3;
			...
			castelX2 += castelSpd2;
			if (castelX2 > 470 || castelX2 < -190)
				castelSpd2 = -castelSpd2;
		}
